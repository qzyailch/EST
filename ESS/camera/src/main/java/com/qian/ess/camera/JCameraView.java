...
import com.qian.ess.camera.state.CameraMachine;
import com.qian.ess.camera.util.FileUtil;
import com.qian.ess.camera.util.LogUtil;
import com.qian.ess.camera.util.ScreenUtils;
import com.qian.ess.camera.view.CameraView;
...
//    private static final String TAG = "JCameraView";
    ...
    //点击按钮
    public static final int TYPE_BTN_MENU = 0x520;
    public static final int TYPE_BTN_CONFIRM = 0x522;
...
//                    Log.i("CJT", "result = " + (result - firstTouchLength));
    ...
    @Override
    public void confirmState(int type, int btnType) {
        switch (type) {
            case TYPE_VIDEO:
                stopVideo();    //停止播放
                mVideoView.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
                machine.start(mVideoView.getHolder(), screenProp);
                if (jCameraLisenter != null) {
                    jCameraLisenter.recordSuccess(videoUrl, firstFrame, btnType);
                }
                break;
            case TYPE_PICTURE:
                mPhoto.setVisibility(INVISIBLE);
                if (jCameraLisenter != null) {
                    jCameraLisenter.captureSuccess(captureBitmap, btnType);
                }
                break;
            case TYPE_SHORT:
                break;
